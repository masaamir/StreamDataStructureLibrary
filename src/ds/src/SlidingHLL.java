...
import ds.util.Element;
import ds.util.ElementList;
import static ds.util.HyperLogLogUtil.computeHash;
...
public class SlidingHLL implements Serializable {
	...
	public int add(long value, long timestamp) {
		...
		boolean changed = preElementList.addNewElement((byte)lowestBitPosition,
				timestamp);
	...
	public boolean merge(int bucketno, int lowestbitposition, long timestamp) {
		...
		boolean ischanged = preElementList.addNewElement((byte)lowestbitposition,
				timestamp);
