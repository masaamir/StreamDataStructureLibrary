...
import ds.util.{BucketAndHash, ElementListScala}
import ds.util.HyperLogLogUtil.computeHash

import scala.util.control.Breaks._

class SlidingHLLScalaWithW(numOfBucket: Int,W:Long) extends Serializable {

  private var buckets: Array[ElementListScala] = new Array[ElementListScala](numOfBucket)
  ...
  def add(value: Long, timestamp: Long): Int = {
    ...
    if (null == this.buckets(bucket)) {
      ...
      //remove all elements that are older than W in respective window
      preElementList.setElemList(preElementList.elemList
        .filter(t=> t._2 < (timestamp - W)))
    }
  ...
  def initializeBucket(numberOfBuckets: Int): Unit = {
    for (i <- 0 until numberOfBuckets) {
      this.buckets.update(i, new ElementListScala)
    }

  }

  def union(b: SlidingHLLScalaWithW): Unit = {
